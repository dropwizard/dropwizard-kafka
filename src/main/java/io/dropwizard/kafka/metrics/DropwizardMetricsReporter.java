...
public class DropwizardMetricsReporter implements MetricsReporter {
    ...
    public void init(final List<KafkaMetric> metrics) {
        if (includeTags) {
            metrics.stream()
                    ...
                    .forEach(metric -> createMetricNamesWithTags(metric.metricName())
                            .forEach(metricName -> tryRegister(metricName, metric::metricValue)));
        } else {
            metrics.stream()
                    .filter(metric -> shouldRegister(metric::metricValue))
                    .forEach(metric -> tryRegister(createMetricName(metric.metricName()), metric::metricValue));
        }
    }
    ...
    List<String> createMetricNamesWithTags(final MetricName metricName) {
        ...
        return metricName.tags()
                .entrySet()
                .stream()
                .map(entry -> name(name, metricName.group(), metricName.name(), entry.getKey(), entry.getValue()))
    ...
    @Override
    public void close() {
        registry.getGauges((metricName, metric) -> metricName.startsWith(name))
                .forEach((metricName, metric) -> registry.remove(metricName));
    }

    @Override
    public void configure(final Map<String, ?> configs) {
        this.includeTags = Optional.ofNullable(configs.get(SHOULD_INCLUDE_TAGS_CONFIG))
                .filter(String.class::isInstance)
                .map(String.class::cast)
                .map(Boolean::parseBoolean)
                .orElse(false);

        this.name = Objects.requireNonNull((String) configs.get(METRICS_NAME_CONFIG),
